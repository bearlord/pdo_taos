...
static int pdo_taos_stmt_execute(pdo_stmt_t *stmt)
{
    ...
    S->result = taos_query(H->server, ZSTR_VAL(stmt->active_query_string));
...
static int pdo_taos_stmt_get_col(pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type)
{
    unsigned long *len;
    char **ptr;
    pdo_taos_stmt *S = (pdo_taos_stmt *) stmt->driver_data;
    ...
    if (S->stmt) {
        ...
        switch (S->fields[colno].type) {
            ...
            case TSDB_DATA_TYPE_TINYINT:
                ZVAL_LONG(result, *((int8_t *) row[colno]));
                break;

            case TSDB_DATA_TYPE_UTINYINT:
                ZVAL_LONG(result, *((uint8_t *) row[colno]));
                break;

            case TSDB_DATA_TYPE_SMALLINT:
                ZVAL_LONG(result, *((int16_t *) row[colno]));
                break;

            case TSDB_DATA_TYPE_USMALLINT:
                ZVAL_LONG(result, *((uint16_t *) row[colno]));
                break;

            case TSDB_DATA_TYPE_INT:
                ZVAL_LONG(result, *((int32_t *) row[colno]));
                break;
            ...
            case TSDB_DATA_TYPE_UINT:
                ZVAL_LONG(result, *((uint32_t *) row[colno]));
                break;
