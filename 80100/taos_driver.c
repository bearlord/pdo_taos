...
/* {{{ */
static void pdo_taos_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info)
{
    pdo_taos_db_handle *H = (pdo_taos_db_handle *) dbh->driver_data;
    pdo_taos_error_info *einfo = &H->einfo;
...
taos_handle_preparer(pdo_dbh_t *dbh, zend_string *sql, pdo_stmt_t *stmt, zval *driver_options)
{
    ...
    int code = taos_stmt_prepare(S->stmt, ZSTR_VAL(sql), 0);
    if (code != 0) {
...
static zend_long taos_handle_doer(pdo_dbh_t *dbh, const zend_string *sql)
{
    ...
    res = taos_query(H->server, ZSTR_VAL(sql));
...
/* {{{ pdo_taos_set_attr */
static bool pdo_taos_set_attr(pdo_dbh_t *dbh, zend_long attr, zval *val)
{
